	<div id="urHere"><?php echo Yii::app()->params['mb_nav_first']?><b>&gt;</b><strong>案例扩展信息管理</strong> </div>
	<div id="mainBox">
	    <h3>
			<a class="actionBtn" href="<?php echo $this->createUrl("goods/index")?>">案例列表</a>&nbsp;
	    	<a class="actionBtn add" href="<?php echo $this->createUrl("goodsinfo/add" , array('ids'=>$ids))?>">添加扩展信息</a>
	   		 案例扩展信息列表
	    </h3>
	    <div class="filter">
	    ...
	    <table cellspacing="0" cellpadding="8" border="0" width="100%" class="tableBasic">
	      <tbody>
	     	...
	     	<tr align="center">
	        	...
	        	<td><?php echo $v->csc_name_en?></td>
				<td class="order" pos="<?php echo $this->createUrl('goodsinfo/order')?>" ids="<?php echo $v->csc_id?>"><?php echo $v->csc_sort ?></td>
	        	<td><?php echo $v->csc_create?></td>
	        	<td><?php echo $v->csc_update?></td>
		        <td>
		        	<a href="<?php echo $this->createUrl('goodsinfo/edit',array('ids'=>$v->csc_id))?>">编辑</a>
					<a href="<?php echo $this->createUrl('goodsinfo/del',array('ids'=>$v->csc_id))?>" class="del" warning="确认删除?">删除</a>
				</td>
			</tr>
			<?php endforeach;?>
		  </tbody>
